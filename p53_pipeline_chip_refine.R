...
# another GRN
if(T){
  GRN_refined = human_dorothea_logic_matrix
  rowSums(GRN_refined)["TP53"]
  ids = match(P53_target_in_chip,colnames(GRN_refined))
  vec = rep(0,ncol(GRN_refined))
  vec[ids] = 1
  GRN_refined["TP53",] = vec
  rowSums(GRN_refined)["TP53"]
  
  
  TFA_IntronCPM=scTFA_calculation(as.matrix(data_IntronCPM),GRN_refined,zscore=F,
                                  gene_weight_method=NULL)
  TFA_ExonCPM=scTFA_calculation(as.matrix(data_ExonCPM),GRN_refined,zscore=F,
                                gene_weight_method=NULL)
